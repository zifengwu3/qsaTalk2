...
struct dev_config {
	...
	unsigned char ip[20];				//192.168.10.XX
	unsigned char mask[20];				//255.255.0.0
	unsigned char route[20];				//192.168.10.1
	unsigned char center_ip[20];			//192.168.10.XX
	unsigned char server_ip[20]; 		//192.168.10.XX
	unsigned char undefined_ip[100];	//192.168.10.XX x 5
	unsigned char undefined_param[100];
};

struct _send_info {
    unsigned char ip[20];		 //发送信息的设备IP地址，需要广告机或上位服务器提供（搜网）
    unsigned char addr[4];		 //发送信息的设备房号地址，4位房号地址,需要广告机或上位服务器提供（搜网）
    char * title[16];		     //发送的信息的标题，需要上位服务器提供
